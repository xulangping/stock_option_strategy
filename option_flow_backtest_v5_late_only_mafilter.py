...
# -*- coding: utf-8 -*-
"""
Option Flow Momentum Strategy Backtest V5 - Late Trading Only (After 3:45 PM)
Only trades signals that occur after 3:45 PM ET
...
class OptionFlowBacktestV5:
    def __init__(self, initial_capital: float = 100000):
        ...
        self.stop_loss = -0.05
        self.blacklist_days = 10  # Don't buy same stock for 5 days
        self.entry_delay = 5  # Delay in minutes for entry
        # Late trading parameters
        self.trade_start_time = (13, 00)  # Only trade after 1:00 PM
        self.exit_time = (13, 00)  # Exit time for next day (same as trade_start_time for better capital turnover)
        self.min_option_premium = 100000  # Minimum option premium threshold (100K)
        self.max_dte = 100  # Maximum days to expiration (filter out options > 40 days)
    ...
    def extract_expiration_from_option_id(self, option_chain_id: str) -> Optional[datetime]:
        ...
        """
        # Extract date pattern YYMMDD from the option chain ID
    ...
    def simulate_position(self, symbol: str, signal_time: datetime, df: pd.DataFrame, option_premium: float) -> bool:
        ...
        # Check if we have next day exit time data before entering position
        next_day_exit = entry_time.replace(hour=self.exit_time[0], minute=self.exit_time[1], second=0) + timedelta(days=1)
        while next_day_exit.weekday() >= 5:
            next_day_exit += timedelta(days=1)
            
        has_next_day_data = False
        for idx in range(len(df)):
            bar = df.iloc[idx]
            bar_time = bar['datetime']
            if (bar_time.date() == next_day_exit.date() and 
                bar_time.hour == self.exit_time[0] and bar_time.minute >= self.exit_time[1]):
                has_next_day_data = True
                break
        
        if not has_next_day_data:
            print(f"  Skipping {symbol} - no next day {self.exit_time[0]:02d}:{self.exit_time[1]:02d} data available")
            return False
        ...
        for idx in range(entry_idx + 1, len(df)):
            ...
            # Check if it's next trading day exit time
            if current_time.date() > entry_time.date() and current_time.hour == self.exit_time[0] and current_time.minute >= self.exit_time[1]:
        ...
        # If we reach here, we need to find the next day exit time price
        next_day_exit = entry_time.replace(hour=self.exit_time[0], minute=self.exit_time[1], second=0) + timedelta(days=1)
        ...
        # Find the price at or after exit time next day
        for idx in range(len(df)):
            bar = df.iloc[idx]
            bar_time = bar['datetime']
            
            # Check if it's the next trading day at or after exit time
            if (bar_time.date() == next_day_exit.date() and 
                bar_time.hour == self.exit_time[0] and bar_time.minute >= self.exit_time[1]):
        ...
        # If no next day exit time data found, skip this trade
        print(f"  [WARNING] No next day {self.exit_time[0]:02d}:{self.exit_time[1]:02d} data found for {symbol}, position remains open")
        return False
    ...
    def run_backtest(self, option_flows_file: str):
        ...
        for idx, row in df_late.iterrows():
            ...
            # Calculate DTE (days to expiration) from signal date
            signal_date = signal_time.date()
    ...
    def calculate_metrics(self):
        ...
        # Display results
        print("\n" + "="*60)
        print("BACKTEST RESULTS - LATE TRADING ONLY (3:45 PM+)")
        print("="*60)
        ...
        print(f"\nTotal Signals Processed: {self.total_signals}")
        ...
        print(f"MA Filter Pass Rate: {(self.total_signals - self.ma_filtered_signals) / self.total_signals:.2%}" if self.total_signals > 0 else "MA Filter Pass Rate: 0%")
        print(f"Premium Filter Pass Rate: {(self.total_signals - self.premium_filtered_signals) / self.total_signals:.2%}" if self.total_signals > 0 else "Premium Filter Pass Rate: 0%")
        print(f"DTE Filter Pass Rate: {(self.total_signals - self.dte_filtered_signals) / self.total_signals:.2%}" if self.total_signals > 0 else "DTE Filter Pass Rate: 0%")
