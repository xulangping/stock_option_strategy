...
# -*- coding: utf-8 -*-
"""
Option Flow Momentum Strategy Backtest V4 - With Late Day Trading Feature
Optimized version using pre-downloaded stock price data
Includes special 3:45 PM trading rule and transaction costs
"""
...
class OptionFlowBacktestV4:
    def __init__(self, initial_capital: float = 100000):
        ...
        self.daily_late_trade_used = {}  # Track if late trade was used today
        ...
        self.late_trade_time = 15, 45  # 3:45 PM
        self.late_trade_position = 0.20  # Fixed 20% position for late trades
    ...
    def is_late_trade_eligible(self, signal_time: datetime) -> bool:
        """Check if this signal is eligible for late day trading"""
        # Check if it's after 3:45 PM
        if signal_time.hour > self.late_trade_time[0] or \
           (signal_time.hour == self.late_trade_time[0] and signal_time.minute >= self.late_trade_time[1]):
            # Check if we haven't used late trade today
            date = signal_time.date()
            if not self.daily_late_trade_used.get(date, False):
                return True
        return False
    ...
    def execute_trade(self, symbol: str, signal_time: datetime, action: str, price: float, shares: int = 0, is_late_trade: bool = False):
        ...
        if action == 'BUY':
            ...
            # Mark late trade as used if applicable
            if is_late_trade:
                self.daily_late_trade_used[trade_date] = True
    ...
    def simulate_position(self, symbol: str, signal_time: datetime, df: pd.DataFrame, option_premium: float, is_late_trade: bool = False) -> bool:
        ...
        if is_late_trade:
            # Fixed 20% position for late trades
            position_pct = self.late_trade_position
        else:
            # Normal position sizing based on option premium
        ...
        # Update daily position tracking (only for non-late trades)
        if not is_late_trade:
            date = signal_time.date()
        ...
        # Execute buy
        trade_type = "[LATE_BUY]" if is_late_trade else "[BUY]"
        print(f"  {trade_type} {symbol}: {shares} shares @ ${entry_price:.2f} at {entry_time} (Position: {position_pct:.1%})")
        self.execute_trade(symbol, entry_time, 'BUY', entry_price, shares, is_late_trade)
        ...
        for idx in range(entry_idx + 1, len(df)):
            ...
            if returns <= self.stop_loss:
                ...
                self.execute_trade(symbol, current_time, 'STOP_LOSS', current_price, shares, is_late_trade)
                return True
            
            # Check take profit
            if returns >= self.take_profit:
                print(f"  [TAKE_PROFIT] {symbol}: {shares} shares @ ${current_price:.2f} at {current_time} (Return: {returns:.2%})")
                self.execute_trade(symbol, current_time, 'TAKE_PROFIT', current_price, shares, is_late_trade)
                return True
            ...
            if current_time.date() > entry_time.date() and current_time.hour == 9 and current_time.minute >= 30:
                ...
                self.execute_trade(symbol, current_time, 'NEXT_DAY_EXIT', current_price, shares, is_late_trade)
                return True
        ...
        for idx in range(len(df)):
            ...
            if (bar_time.date() == next_day_930.date() and 
                ...
                self.execute_trade(symbol, bar['datetime'], 'NEXT_DAY_EXIT', bar['close'], shares, is_late_trade)
                return True
    ...
    def run_backtest(self, option_flows_file: str):
        ...
        for idx, row in df.iterrows():
            ...
            # Skip if signal is too close to market close (after 3:58 PM) - unless it's a late trade candidate
            if signal_time.hour == 15 and signal_time.minute >= 58:
                if not self.is_late_trade_eligible(signal_time):
            ...
            # Check if this is a late trade opportunity
            is_late_trade = self.is_late_trade_eligible(signal_time)
            
            if is_late_trade:
                print(f"  [LATE TRADE OPPORTUNITY] Signal after 3:45 PM")
            else:
                # Check if can open normal position
            ...
            if not stock_data.empty:
                ...
                self.simulate_position(symbol, signal_time, stock_data, option_premium, is_late_trade)
            else:
    ...
    def calculate_metrics(self):
        ...
        # Separate late trades
        late_trades = [t for t in completed_trades if t.get('is_late_trade', False)]
        regular_trades = [t for t in completed_trades if not t.get('is_late_trade', False)]
        ...
        for trade in self.trades_history:
            if trade['action'] == 'BUY':
                current_equity -= trade['net_value']
            else:
                current_equity += trade['net_value']
        ...
        print(f"  Late Trades (3:45 PM+): {len(late_trades)}")
        ...
        # Late trade analysis
        if late_trades:
            late_wins = [t for t in late_trades if t['net_pnl'] > 0]
            late_win_rate = len(late_wins) / len(late_trades)
            late_avg_return = np.mean([t['net_return'] for t in late_trades])
            print(f"\nLate Trade Analysis:")
            print(f"  Total Late Trades: {len(late_trades)}")
            print(f"  Late Trade Win Rate: {late_win_rate:.2%}")
            print(f"  Late Trade Avg Return: {late_avg_return:.2%}")
        
        # Trade breakdown by exit type
        exit_types = {}
        for trade in self.trades_history:
            if trade['action'] in ['STOP_LOSS', 'TAKE_PROFIT', 'NEXT_DAY_EXIT']:
        ...
        if self.trades_history:
            ...
            trade_df.to_csv('backtest_trades_v4_late_trade.csv', index=False)
            print(f"\nDetailed trade log saved to 'backtest_trades_v4_late_trade.csv'")
...
def main():
    # Initialize backtest
    backtest = OptionFlowBacktestV4(initial_capital=100000)
