...
class OptionFlowBacktestV5:
    ...
    def simulate_position(self, symbol: str, signal_time: datetime, df: pd.DataFrame, option_premium: float) -> bool:
        ...
        for idx in range(entry_idx + 1, len(df)):
            ...
            # Check if it's next trading day 3:00 PM
            if current_time.date() > entry_time.date() and current_time.hour == 15 and current_time.minute >= 0:
        ...
        # If we reach here, we need to find the next day 3:00 PM price
        next_day_300pm = entry_time.replace(hour=15, minute=0, second=0) + timedelta(days=1)
        ...
        # Find the price at or after 3:00 PM next day
        for idx in range(len(df)):
            bar = df.iloc[idx]
            bar_time = bar['datetime']
            
            # Check if it's the next trading day at or after 3:00 PM
            if (bar_time.date() == next_day_300pm.date() and 
                bar_time.hour == 15 and bar_time.minute >= 0):
        ...
        # If no next day 3:00 PM data found, skip this trade
        print(f"  [WARNING] No next day 3:00 PM data found for {symbol}, position remains open")
        return False
