...
# -*- coding: utf-8 -*-
"""
Option Flow Momentum Strategy Backtest V2
Using Polygon Python client for better API handling
"""
...
class OptionFlowBacktestV2:
    def __init__(self, initial_capital: float = 100000):
        ...
        self.client = RESTClient("PzwHlhhXCVYIlLLbAybDJGahQm5LbheO")
    ...
    def get_stock_data_for_period(self, symbol: str, signal_time: datetime) -> pd.DataFrame:
        """Get all minute data from signal time to next day open"""
        start_date = signal_time.strftime("%Y-%m-%d")
        
        # Get next trading day open time
        next_day_open = self.get_next_trading_day_open(signal_time)
        end_date = next_day_open.strftime("%Y-%m-%d")
        ...
        try:
            ...
            for agg in self.client.list_aggs(
                ...
                aggs.append({
                    # Convert UTC timestamp to Eastern Time (UTC-4 or UTC-5)
                    'datetime': datetime.fromtimestamp(agg.timestamp / 1000, tz=timezone.utc).astimezone(timezone(timedelta(hours=-4))).replace(tzinfo=None),
                    'open': agg.open,
                    'high': agg.high,
                    'low': agg.low,
                    'close': agg.close,
        ...
        time.sleep(12.0)  # Much longer delay for free tier API limits
        return pd.DataFrame()
    ...
    def simulate_position(self, symbol: str, signal_time: datetime, df: pd.DataFrame, option_premium: float) -> bool:
        ...
        # Note: In real trading, the position would remain open
        # For backtest purposes, we'll leave it in positions without exit
        return False
    ...
    def run_backtest(self, option_flows_file: str):
        ...
        for idx, row in df.iterrows():
            ...
            # Get all data for the trading period
            stock_data = self.get_stock_data_for_period(symbol, signal_time)
    ...
    def can_open_position(self, date: datetime.date) -> bool:
        ...
        # Check daily trade limit (only first 4 trades per day)
    ...
    def calculate_metrics(self):
        ...
        if self.trades_history:
            ...
            trade_df.to_csv('backtest_trades_v2.csv', index=False)
            print(f"\nDetailed trade log saved to 'backtest_trades_v2.csv'")
...
def main():
    ...
    # Run backtest
    option_file = "option_data/flow_true_mid_side,no_side,bid_side_false_true_Common Stock_50_60_0.8_100000_5_Tue Sep 16 2025 00_00_00 GMT+0800 (中国标准时间)_Fri Sep 19 2025 23_59_00 GMT+0800 (中国标准时间)_true_true_Calls_true_strategy_v1_1000000000000.csv"
