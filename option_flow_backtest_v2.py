...
# -*- coding: utf-8 -*-
"""
Option Flow Momentum Strategy Backtest V5 - Friday Late Trading Only (After 3:30 PM)
Only trades signals that occur on Friday after 3:30 PM ET
Holds positions for configurable number of trading days
Maximum 4 trades per day with proportional position sizing
"""
...
class OptionFlowBacktestV5:
    def __init__(self, initial_capital: float = 100000, holding_days: int = 1):
        ...
        # Strategy parameters - Late trading only
        ...
        self.holding_days = holding_days  # Number of trading days to hold position
    ...
    def is_late_trade_time(self, signal_time: datetime) -> bool:
        """Check if signal is after 3:45 PM"""
    ...
    def simulate_position(self, symbol: str, signal_time: datetime, df: pd.DataFrame, option_premium: float) -> bool:
        ...
        # Check if we have next day 3:00 PM data before entering position
        next_day_300pm = entry_time.replace(hour=15, minute=0, second=0) + timedelta(days=1)
        while next_day_300pm.weekday() >= 5:
            next_day_300pm += timedelta(days=1)
            
        has_next_day_data = False
        for idx in range(len(df)):
            bar = df.iloc[idx]
            bar_time = bar['datetime']
            if (bar_time.date() == next_day_300pm.date() and 
                bar_time.hour == 15 and bar_time.minute >= 0):
                has_next_day_data = True
                break
        
        if not has_next_day_data:
            print(f"  Skipping {symbol} - no next day 3:00 PM data available")
            return False
        ...
        for idx in range(entry_idx + 1, len(df)):
            ...
            # Check if it's time to exit based on holding_days
            ...
            # Exit at 3:00 PM after holding for specified trading days
            if trading_days_held >= self.holding_days and current_time.hour == 15 and current_time.minute >= 0:
                print(f"  [HOLDING_PERIOD_EXIT] {symbol}: {shares} shares @ ${current_price:.2f} at {current_time} (Return: {returns:.2%}) after {self.holding_days} trading days")
                self.execute_trade(symbol, current_time, 'HOLDING_PERIOD_EXIT', current_price, shares)
                return True
        
        # If we reach here, we need to find the exit day 3:00 PM price
        # Calculate target exit date based on holding_days
        target_exit_date = entry_time.date()
        trading_days_added = 0
        while trading_days_added < self.holding_days:
            target_exit_date += timedelta(days=1)
            # Skip weekends
            if target_exit_date.weekday() < 5:  # Monday = 0, Friday = 4
                trading_days_added += 1
        
        # Find the price at or after 3:00 PM on target exit day
        for idx in range(len(df)):
            bar = df.iloc[idx]
            bar_time = bar['datetime']
            
            # Check if it's the target exit day at or after 3:00 PM
            if (bar_time.date() == target_exit_date and 
                bar_time.hour == 15 and bar_time.minute >= 0):
                print(f"  [HOLDING_PERIOD_EXIT] {symbol}: {shares} shares @ ${bar['close']:.2f} at {bar['datetime']} (Return: {(bar['close'] - entry_price) / entry_price:.2%}) after {self.holding_days} trading days")
                self.execute_trade(symbol, bar['datetime'], 'HOLDING_PERIOD_EXIT', bar['close'], shares)
                return True
        
        # If no exit day 3:00 PM data found, skip this trade
        print(f"  [WARNING] No exit data found for {symbol} after {self.holding_days} trading days, position remains open")
        return False
    ...
    def run_backtest(self, option_flows_file: str):
        ...
        # Filter for signals after 3:45 PM only
        print(f"\nTotal signals: {len(df)}")
        df_late = df[df['market_time'].apply(self.is_late_trade_time)]
        print(f"Signals after 3:45 PM: {len(df_late)}")
        
        print(f"\nProcessing {len(df_late)} late-day option flow signals...\n")
        
        # Process each option flow signal in chronological order
        for idx, row in df_late.iterrows():
            print(f"\n{'='*60}")
            print(f"Late Signal: {row['underlying_symbol']}")
            ...
            if signal_time.weekday() != 4:
                print(f"  Skipping - Not Friday (weekday: {signal_time.weekday()})")
                continue
    ...
    def can_open_position(self, date: datetime.date) -> bool:
        ...
        # Check daily trade limit (only 4 trades per day)
    ...
    def calculate_metrics(self):
        ...
        # Display results
        print("\n" + "="*60)
        print(f"BACKTEST RESULTS - FRIDAY LATE TRADING (3:30 PM+) WITH {self.holding_days} TRADING DAY(S) HOLD")
        print("="*60)
        ...
        if self.trades_history:
            ...
            trade_df.to_csv('backtest_trades_v5_late_only.csv', index=False)
            print(f"\nDetailed trade log saved to 'backtest_trades_v5_late_only.csv'")
...
def main():
    # Initialize backtest with configurable holding days
    # holding_days=1 means hold for 1 trading day (sell next trading day at 3PM)
    # holding_days=5 means hold for 5 trading days (sell after a week at 3PM)
    holding_days = 2  # Change this to test different holding periods
    backtest = OptionFlowBacktestV5(initial_capital=100000, holding_days=holding_days)
    
    print(f"\nRunning backtest with {holding_days} trading day(s) holding period")
    print(f"Buy on Friday after 3:30 PM, sell after {holding_days} trading day(s) at 3:00 PM\n")
